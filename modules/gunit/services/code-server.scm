(define-module (gunit services code-server)
...
(define (code-server-service config)
  ...
  (list (shepherd-service (documentation "Run the code-server backend.")
...
(define code-server-service-type
  (service-type (name 'code-server)
                ...
                (default-value #t)
                (description
                 "A user service to run a VS Code instance in the browser.")))