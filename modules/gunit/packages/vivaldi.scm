  ...
  (let* ((name (string-append "vivaldi" "-" repo))
    ...
    (package
      ...
      (source
       (origin
         (method url-fetch)
         (uri (string-append "https://downloads.vivaldi.com/stable/" name "_"
                             version "-1_amd64.deb"))
         (sha256
          (base32 hash))))
       ...
       (list
        ...
        #~(modify-phases %standard-phases
            (add-before 'install 'patch-assets
              ...
              (lambda _
                (let* ((bin (string-append #$output "/bin"))
                       ...
                       (opt "./opt")
                       (usr/share "./usr/share")
                       (old-exe (string-append "/opt/vivaldi/"
                                               #$appname))
            ...
            (add-before 'install-wrapper 'install-exe
              (lambda _
                (let* ((bin (string-append #$output "/bin"))
                       (exe (string-append bin "/"
                                           #$appname))
                       (share (string-append #$output "/share"))
                       (chromium-target (string-append #$output
                                                       "/share/vivaldi/"
                                                       #$appname)))
                  (mkdir-p bin)
      ...
      (description
       "Vivaldi is a highly customizable browser developed by Vivaldi.")
      (home-page "https://vivaldi.com/")
      (license (nonfree "https://vivaldi.com/privacy/browser/")))))

(define-public vivaldi-stable
  (make-vivaldi "stable" "7.1.3570.54"
                "1jslsckrv8xwnc4xlrxjiqqpkb74fz51r4yp92p5lr5zj0iayvkh"))
