...
(define-public az
  (package
    ...
    (source (origin
              (method url-fetch)
              (uri (string-append 
                    "https://packages.microsoft.com/repos/" name "/pool/main/a/" name "/"
                    name "_" version "~" "bookworm_amd64.deb"))
                    (file-name (string-append name "-" version "_bookworm_amd64.deb"))
              (sha256
        (base32
         "045mx5j2hhrbh54brg6w2z9v59x3yzapq474k2iz2f3virfkhchz"))))
...
(add-after 'install 'patch-python
  (lambda* (#:key outputs inputs #:allow-other-keys)
    ...
    (synopsis "Azure CLI")
    (description "Azure CLI is a CLI tool for interacting with the Azure cloud platform.")
...
(define azure-cli-libs
  `(
    ...
    ("openssl" ,openssl)
    ("libffi" ,libffi)
    ("util-linux" ,util-linux)   ; for libuuid.so.1 etc.
    ("gcc" ,gcc "lib")           ; for libstdc++.so.6
  ))

(define azure-cli-ld.so.conf
  (packages->ld.so.conf
   (list (fhs-union `(,@azure-cli-libs
                      ,@fhs-min-libs)
                    #:name "fhs-union-64"))))
...
(define-public azure-cli (nonguix-container->package azure-cli-container))
